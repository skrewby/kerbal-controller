// Taken directly from
// https://github.com/Simpit-team/KerbalSimpitRevamped-Arduino/blob/main/src/KerbalSimpitMessageTypes.h
...
enum OutboundPackets {
  ...
  /** Liquid fuel (KSP1) or Methane (KSP2) in the vessel.
       Messages on this channel contain a resourceMessage. Need ARP to work for
     KSP1. */
  LF_MESSAGE = 10,
  ...
  /** TAC Life Support ressources. Only works in KSP1.
      Messages on this channel contain a TACLSRessourceMessage. Need ARP and
     TACLS to work.*/
  TACLS_RESOURCE_MESSAGE = 30,
  /** TAC Life Support waste ressources. Only works in KSP1.
      Messages on this channel contain a TACLSWasteMessage. Need ARP and TACLS
     to work.*/
  TACLS_WASTE_MESSAGE = 31,
  /** Custom ressources. Only works in KSP1. Specific ressource to use to be set
  in the configuration file. Messages on this channel contain a
  CustomResourceMessage. Need ARP and Community Resource Pack to work.*/
  ...
  ORBIT_MESSAGE = 36,
  /** Data about the vessel's rotation (including velocities). This is an
     output, not be confused with rotationMessage, an input from joysticks.
   ...
   *  Messages on this channel contain an rotationMessage, with the mask field
   * unused.
   */
  ROTATION_CMD_MESSAGE = 47,
  /** Translation command after applying Simpit command.
   *  Messages on this channel contain an translationMessage, with the mask
   * field unused.
   */
  TRANSLATION_CMD_MESSAGE = 48,
  /** Wheel command after applying Simpit command.
   *  Messages on this channel contain an wheelMessage, with the mask field
   * unused.
   */
   ...
   *  Messages on this channel contain an throttleMessage (i.e. a single 16-bit
   * integer).
   */
  ...
  /** Action groups.
      ...
      (like Solar and Radiators) are missing. Use ADVANCED_ACTIONSTATUS_MESSAGE
     for those. Messages on this channel contain a single byte representing the
      currently active action groups. A given action group can be checked
      by performing a
      <a href="http://playground.arduino.cc/Code/BitMath#bitwise_and">bitwise
     AND</a> with the message. For example:
  ...
  */
  ...
  /** Current maximum temperature of the any vessel part.
          ...
          skin temperature. The two maximum percentage can come from different
     parts. Messages on this channel contain an tempLimitMessage. */
  ...
  /** Scene change packets are sent by the plugin when
   ...
   This message has a single byte : 0 when a flight is ready, 1 when exiting a
   flight scene.*/
  ...
  Messages on this channel contain a single byte indicating the origin of the
  change : 1 for vessel switching 2 for docking 3 for undocking. */
  ...
  /** Information about intersects with a target orbit.
      ...
      active vessel and a target. Messages on this channel contain a
     intersectsMessage. Only available in KSP2. */
...
*/
enum InboundPackets {
  /* Packet ID used to ask the computer to close the serial connection to this
     controller **/
  CLOSE_SERIAL = 7,
  ...
  /** Request a new message to be sent on this channel (used for channel that
   * only send message on request). If requested on the channel 0, it will
   * trigger a new message on all subscribed channels. May not work on all
   * channels. */
  REQUEST_MESSAGE = 29,
  ...
  */
  ...
  /** Set one of the standard Action Group(s).
      ...
      with the first 6 bits as which action group to set as in the
     AdvancedActionGroupIndexes enum and the last 2 bits the state/action of the
     action group as in the ActionGroupSettings enum in the AutopilotMode
     enum.*/
  SETSINGLE_AG_MESSAGE = 58,
  /** Set one of the custom Action Group(s). */
  SETSINGLE_CAG_MESSAGE = 59,
  /** Send vessel rotation commands.
          If set and not 0, those command will supersede SAS command. The SAS
     will appear not the work when the jostick is in use. If this is the case
     for you, make sure you are sending 0 command to KSP on all used axis.*/
  ROTATION_MESSAGE = 16,
  /** Send vessel translation commands.
              Works only for vessels in KSP1. Use keyboard emulation when on
     EVA. Works for vessels and Kerbals on EVA in KSP2. */
  ...
  /** Send a timewarp TO commands. This is to timewarp to a given time.
     The message (see timewarpToMessage struct) contains a single byte for the
     specific point to aim (periapsis, maneuver node, etc.) and a delay in
     seconds with respect to this point. This allow to timewarp 1h before the
     next morning, 30s before periapsis or 1 min before the start of the burn of
     the next maneuver.*/
  ...
  /** Send a message to emulate a key press.
      ...
      The key code indicate which key to press. It is taken from the file named
     VirtualKeyCode in the folder
     KerbalSimpit\Contrib\InputSimulator\WindowsInput\Native. For instance :
        - 0x4D for 'm' to open the map
        - 0x74 for 'F5' to quicksave
      The modifier is used to emulate key press while holding keys such as CTRL,
     SHIFT, etc This channel only works on Windows, not on Mac or Linux.

      Another alternative is to use the keyboard.h library, which works on all
     operating systems (Windows, Linux, Mac), but needs a specific Arduino board
     (based on ATmega32U4 such as the Due or Leonardo). See the
     KerbalSimpitTranslationForShipAndEVADemo example for instance.
   */
...
/** Handshake values
    These are used to communicate the version of KSP the controller is connected
   to. */
...
/** Action Group Settings for single action groups
    These are used to set the values in the SETSINGLE_AG_MESSAGE and
   SETSINGLE_CAG_MESSAGE. */
...
/** Advanced Action Group States
   ...
   advancedActionStatusMessage::get_action_status in the
   ADVANCED_ACTIONSTATUS_MESSAGE. */
enum AdvancedActionGroupStates {
  /** Value if the action group is not available.
      Use like this:
     if(myAdvancedActionStatusMessage.getActionStatus(ADVANCED_LIGHT_ACTION) ==
     ADVANCED_AG_STATE_NOT_AVAILABLE) */
  ADVANCED_AG_STATE_NOT_AVAILABLE = 0,
  /** Value if the action group is on.
      Use like this:
     if(myAdvancedActionStatusMessage.getActionStatus(ADVANCED_LIGHT_ACTION) ==
     ADVANCED_AG_STATE_ON) */
  ADVANCED_AG_STATE_ON = 1,
  /** Value if the action group is off.
      Use like this:
     if(myAdvancedActionStatusMessage.getActionStatus(ADVANCED_LIGHT_ACTION) ==
     ADVANCED_AG_STATE_OFF) */
  ADVANCED_AG_STATE_OFF = 2,
  /** Value if the action group is mixed, meaning some parts associated with it
     are on and others are off. Use like this:
     if(myAdvancedActionStatusMessage.getActionStatus(ADVANCED_LIGHT_ACTION) ==
     ADVANCED_AG_STATE_MIXED) */
  ADVANCED_AG_STATE_MIXED = 3,
  /** This bit is on if there is science available, it mirrors the flashing blue
     light in the UI. Use it like this:
     if((myAdvancedActionStatusMessage.getActionStatus(ADVANCED_SCIENCE_ACTION)
     & ADVANCED_AG_STATE_BITMASK_SCIENCE_AVAILABLE) != 0) */
  ADVANCED_AG_STATE_BITMASK_SCIENCE_AVAILABLE = 1,
  /** This bit is on if a science experiment is running, the same as when the
     flask is blinking green. Use it like this:
     if((myAdvancedActionStatusMessage.getActionStatus(ADVANCED_SCIENCE_ACTION)
     & ADVANCED_AG_STATE_BITMASK_EXPERIMENTS_IN_PROGRESS) != 0) */
...
/** Timewarp command
    ...
    TIMEWARP_UP, TIMEWARP_DOWN and TIMEWARP_CANCEL_AUTOWARP work for both KSP1
   and KSP2 Use the TIMEWARP_KSP2_XN for setting KSP2 timewarp rates as those
   rates are different. */
enum Timewarp {
  /** Set Timewarp to x1. */
  TIMEWARP_X1 = 0,
...
/** Timewarp command
    ...
    Use the TIMEWARP_XN for setting KSP1 timewarp rates as those rates are
   different. TIMEWARP_UP, TIMEWARP_DOWN and TIMEWARP_CANCEL_AUTOWARP from the
   KSP1 set of Timewarp rates work for both KSP1 and KSP2. */
enum TimewarpKsp2 {
...
enum TimewarpTo {
  ...
  /** Warp to the start of the burn of the next maneuver, estiming the burntime
     using KSP estimations. It is recommended to add a delay to account for the
     needed rotation. */
...
enum CustomLogStatus {
  /** If set, the message will only be put in the KSP log is the Simpit mod is
     in verbose mode */
  VERBOSE_ONLY = 1,
  /** If set, the message will also be displayed to the user (and not only in
     the log) */
  PRINT_TO_SCREEN = 2,
...
/** Rotation Axes
    These are used to indicate which axes in a ROTATION_MESSAGE or
   CAMERA_ROTATION_MESSAGE are active. */
enum RotationAxes {
  PITCH_ROT = 1, /**< Bitmask for the pitch axis. */
  ROLL_ROT = 2,  /**< Bitmask for the roll axis. */
  YAW_ROT = 4,   /**< Bitmask for the yaw axis. */
  ZOOM_ROT =
      8 /**< Bitmask for the zoom axis. Added here purely for camera control. */
};
...
enum FligthStatusFlags {
  FLIGHT_IN_FLIGHT =
      1,             /**< True if the game is currently in the flight screen. */
  FLIGHT_IS_EVA = 2, /**< True if the current flight is an EVA. */
  FLIGHT_IS_RECOVERABLE = 4, /**< True if the current vesset is recoverable. */
  FLIGHT_IS_ATMO_TW = 8,     /**< True if the current Timewarp mode is for
                                atmosphere (i.e. physical timewarp). */
  FLIGHT_CONTROL_LEVEL_0 =
      16, /**< 2 bits used to code the Control Level. Use the helper function of
             flightStatusMessage to parse it. */
  FLIGHT_CONTROL_LEVEL_1 =
      32, /**< 2 bits used to code the Control Level. Use the helper function of
             flightStatusMessage to parse it. */
