...
void uart_setup(uint32_t system_core_clock, uint32_t baud_rate) {
  ...
  uint16_t uartdiv = system_core_clock / baud_rate;
  USART2->BRR = uartdiv;
  USART2->CR1 |=
      (USART_CR1_RE | USART_CR1_TE | USART_CR1_UE | USART_CR1_RXNEIE);
...
int uart_read_available() { return ringbuffer_has_data(&buffer); }
static int read_available() { return USART2->ISR & USART_ISR_RXNE; }
...
static uint8_t read() { return USART2->RDR & 0xFF; }

void uart_empty_buffer() { ringbuffer_init(&buffer); }
...
int __io_getchar(void) { return uart_read(); }

void USART2_IRQHandler() {
  if (read_available()) {
    uint8_t value = read();
    ringbuffer_write(&buffer, &value, 1);
  }
}
